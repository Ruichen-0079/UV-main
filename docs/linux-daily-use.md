...
| Local STT | Packaged Linux install (`pnpm desktop:package:linux-daily`): Supervisor starts `local-stt/yuvi-local-stt` from the immutable artifact, with models next to the sidecar and speaker profiles under the existing YUVI data root. No venv, `pnpm local-stt:provision`, or `YUVI_LOCAL_STT_START_COMMAND` is required. Checkout daily-use may still set `YUVI_AUTOSTART_LOCAL_STT=true` and `YUVI_LOCAL_STT_START_COMMAND` after `pnpm local-stt:provision`. Set `YUVI_STT_SPEAKER_DIR` explicitly only to retain an existing store. |

 ...
 * Does not commit weights. Records SHA256 in services/local-stt/models.manifest.json.
 */
...
const assets = [
  {
    ...
    archive: true,
    keep: ["sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/model.int8.onnx", "sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/tokens.txt", "sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/LICENSE", "sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/test_wavs"],
    drop: ["sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17/model.onnx"]
  },
  {
...
for (const asset of assets) {
  ...
  if (asset.archive) {
    execFileSync("tar", ["-xjf", packed, "-C", dest], { stdio: "inherit" });
  } else {
    fs.copyFileSync(packed, path.join(dest, name));
  }
}

const float32 = path.join(dest, "sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17", "model.onnx");
if (fs.existsSync(float32)) fs.rmSync(float32);
...
for (const model of manifest.models) {
  ...
  model.sha256 = sha256(filePath);
  model.bytes = stat.size;
  console.log(`${model.id} sha256=${model.sha256} bytes=${model.bytes}`);
}

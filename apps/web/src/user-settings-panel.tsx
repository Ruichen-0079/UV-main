        ...
        {(!props.sections || props.sections.includes("memory")) && (
          <section className="settings-card">
            ...
            <p className="mt-2 text-xs text-ink-500">
              {t(
                "Identity scope is managed from People & voices. Legacy subject and persona values are retained for compatibility and are not edited here."
              )}
            </p>
            ...
            <div className="mt-4 border-t border-ink-200 pt-4">
              ...
              <p className="mt-1 text-xs text-ink-500">
                {t(
                  "This LLM key is independent from the Chat DeepSeek key. It is only used for memory inference when Memory is Enabled, Managed, and Mem0."
                )}
              </p>
            </div>
            ...
            {form.memoryLlmProvider === "none" ? (
              <p className="mt-2 text-xs text-ink-500">
                {t(
                  "Memory inference disabled. Model and base URL values can be retained without being sent to Mem0."
                )}
              </p>
            ) : null}
            {form.memoryLlmProvider !== "none" && !form.memoryLlmModel.trim() ? (
              <p className="mt-2 text-xs text-red-600">
                {t("Select a provider and enter a model name before saving.")}
              </p>
            ) : null}
            {form.memoryLlmProvider !== "none" &&
            !state.secrets.memoryLlmApiKey &&
            !form.memoryLlmApiKeyInput.trim() ? (
              <p className="mt-2 text-xs text-ink-500">
                {t("API key not configured; infer=true will remain unavailable.")}
              </p>
            ) : null}
            {!form.memoryEnabled ||
        ...
        {(!props.sections || props.sections.includes("desktop")) && (
          <section className="settings-card">
            ...
            <label className="setting-checkbox">
              ...
              />
              {t("Start the local service automatically")}
            </label>

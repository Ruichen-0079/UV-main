...
export function MemoryPage(props: {
      ...
      void apiClient
        ...
        .catch((caught) => {
          if (!controller.signal.aborted) {
            setSearchError(caught instanceof Error ? caught.message : t("Memory search failed"));
  ...
  async function createMemory(): Promise<void> {
    if (creating || !createForm.content.trim()) {
      return;
    }
  ...
  async function updateMemoryLifecycle(
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : t("Memory {0} failed", action));
    } finally {
      setBusyMemoryId(null);
    }
  }
  ...
  return (
    <PageShell title={t("Memory")} subtitle="Manual memory management console for development.">
      <div className="grid grid-cols-3 gap-4">
        <StatusCard title={t("Repository")} status={memoryMode} detail={memoryModeDetail(memoryMode)} />
        <StatusCard
          title={t("Result Source")}
          status={resultSource}
          detail={
            query.trim()
              ? t("Search active · mode: {0} · excluded status/time/scope: {1}/{2}/{3}", searchRetrievalMode ?? "unknown", searchExclusions.status ?? 0, searchExclusions.time ?? 0, searchExclusions.scope ?? 0)
      ...
      {creating && <AsyncProgress label={t("Creating memory…")} />}
      {maintenanceBusy && <AsyncProgress label={t("Memory maintenance in progress…")} />}
      <Notice
        tone="info"
        title={t("Memory mode")}
        message={t("This development console edits repository records; Runtime semantic Memory remains owned by its configured backend. in-memory resets on server restart. postgres persists after DATABASE_URL is configured and pnpm db:migrate has been applied. Do not store secrets in memory.")}
      />
      <Panel title={t("Memory Health")} badge={maintenanceHealth.data?.repository ?? memoryMode}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          >{t("Run Maintenance Dry Run")}</button>
          <button
            className="button-secondary"
            type="button"
            disabled={maintenanceBusy}
            onClick={() => void runMaintenance(false)}
          >{t("Run Maintenance")}</button>
        ...
        {maintenanceResult && (
          <div className="mt-3 rounded-md border border-ink-100 bg-ink-50 p-3 text-xs text-ink-600">
            {t("scanned={0} expired={1} stale={2} supersessionWarnings={3} skipped={4} failed={5}", maintenanceResult.scanned, maintenanceResult.expired, maintenanceResult.stale, maintenanceResult.supersessionWarnings, maintenanceResult.skipped, maintenanceResult.failed)}
          </div>
        )}
      </Panel>
      <Panel
        title={t("ANN Vector Index")}
      ...
      >
        ...
        {vectorIndexStatus.data?.status.indexFallbackReason && (
          <Notice
            tone="info"
            title={t("ANN fallback")}
            message={t("{0} Retrieval still works without ANN acceleration.", vectorIndexStatus.data.status.indexFallbackReason)}
          />
        )}
      </Panel>
      <div className="grid grid-cols-[1fr_340px] gap-4">
        <Panel title={t("Memory Records")}>
          <div className="mb-3 grid grid-cols-[1fr_150px_170px_150px_150px_auto] gap-3">
            <input
              className="field"
              placeholder={t("Search memory content")}
              value={query}
            ...
            >
              <option value="all">{t("All subtypes")}</option>
            ...
            <input
              ...
              aria-label={t("Minimum importance")}
            />
            <button className="button-secondary" type="button" onClick={clearFilters}>{t("Clear")}</button>
          </div>
          <div className="mb-3 grid grid-cols-[140px_1fr_160px_140px_140px_100px_110px_90px] gap-3">
            ...
            <input
              className="field"
              placeholder={t("scopeId, e.g. yuvi-runtime")}
              value={scopeIdFilter}
          ...
          <div className="mb-3 grid grid-cols-[150px_180px_180px_1fr] gap-3">
            ...
            >
              <option value="all">{t("All embeddings")}</option>
              <option value="embedded">{t("Embedded")}</option>
              <option value="missing">{t("Missing embedding")}</option>
            </select>
            <select
              className="field"
            ...
            >
              <option value="all">{t("All embedding providers")}</option>
            ...
            >
              <option value="all">{t("All embedding models")}</option>
            ...
            <div className="rounded-md border border-ink-100 bg-ink-50 px-3 py-2 text-xs text-ink-600">{t("Re-embed existing Postgres memories with")}{" "}
          ...
          {props.state.error && (
            <Notice tone="error" title={t("Memory load failed")} message={props.state.error} />
          )}
          {searchError && (
            <Notice
              tone="error"
              title={t("Memory search failed")}
              message={t("{0}. Showing local recent-memory fallback if available.", searchError)}
            />
          )}
          {success && <Notice tone="info" title={t("Saved")} message={success} />}
          {error && <Notice tone="error" title={t("Memory action failed")} message={error} />}
          {!props.state.loading && memories.length === 0 ? (
            <EmptyState
              title={t("No matching memories")}
              message={t("Create a memory or adjust the filter.")}
            />
          ) : (
            <MemoryTable
              memories={memories}
      ...
      <div className="grid grid-cols-2 gap-4">
        <Panel title="最近候选记忆" badge="调试">
          ...
          {candidates.loading && (
            <Notice tone="info" title={t("Loading")} message={t("Fetching recent extraction candidates.")} />
          )}
          {candidates.error && (
            <Notice tone="error" title={t("Candidate load failed")} message={candidates.error} />
          )}
          {!candidates.loading && (candidates.data?.candidates.length ?? 0) === 0 ? (
            <EmptyState
              title={t("No recent candidates")}
              message={t("Send a message with Write Memory enabled to see extractor suggestions.")}
            />
          ) : (
            <MemoryCandidateList
        ...
        <Panel title={t("Memory Details")}>
          {detailLoading && (
            <Notice tone="info" title={t("Loading")} message={t("Fetching memory detail.")} />
          )}
          {!selectedMemory && !detailLoading ? (
            <EmptyState title={t("No memory selected")} message={t("Use View to inspect a memory.")} />
          ) : selectedMemory ? (
            <MemoryDetail memory={selectedMemory} />
          ) : null}
        </Panel>
        <Panel title={t("Edit / Delete")}>
          {editingCandidate ? (
            <div>
              <div className="mb-3 rounded-md border border-ink-100 bg-ink-50 p-3 text-xs text-ink-600">{t("Editing candidate")}{" "}{shortTrace(editingCandidate.id)}{t("from trace")}{" "}
            ...
            <div className="space-y-3">
              <Notice
                ...
                message={t("Deletion cannot be undone in this development console.")}
              />
            ...
            <EmptyState
              title={t("No edit active")}
              message={t("Use Edit or Delete from the memory table.")}
            />
          )}
        </Panel>
      </div>
...
function memoryHealthEntries(
  ...
  return [
    { label: t("active"), value: health?.active ?? 0 },
    { label: t("expired"), value: health?.expired ?? 0 },
    { label: t("archived"), value: health?.archived ?? 0 },
    { label: t("superseded"), value: health?.superseded ?? 0 },
    { label: t("forgotten"), value: health?.forgotten ?? 0 },
...
function memoryMaintenanceStatusEntries(
  ...
  return [
    ...
    { label: "interval", value: status ? t("{0} min", status.intervalMinutes) : "0 min" },
    ...
    { label: "next run", value: formatDate(status?.nextRunAt ?? "") },
    {
      ...
      value: status?.lastSummary
        ? t("expired={0} stale={1} failed={2}", status.lastSummary.expired, status.lastSummary.stale, status.lastSummary.failed)
        : status?.lastError
...
function vectorIndexEntries(
  ...
  return [
    ...
    { label: t("enabled"), value: status?.vectorIndexEnabled ? "true" : "false" },
    ...
    { label: t("missing"), value: String(status?.missingEmbeddingCount ?? 0) },
...
export function emptyMemoryForm(): MemoryForm {
  return {
    type: "semantic",
    subtype: "",
    scope: "user",
...
export function toCreateMemoryRequest(form: MemoryForm): CreateMemoryRequest {
  ...
  return (
    <div className="space-y-3">
      ...
      {temporalWarning && (
        <Notice
          tone="info"
          title={t("Relative time detected")}
          message={t("Relative time detected. Consider resolving it to an absolute date before saving.{0}", temporalWarning ? ` Suggested: ${temporalWarning}` : "")}
        />
      )}
...
function MemoryDetail(props: { memory: MemoryRecord }): JSX.Element {
  ...
  return (
    <div className="space-y-3 text-sm">
      <div className="grid grid-cols-3 gap-3">
        <Definition label={t("Type")} value={props.memory.type} />
        <Definition label={t("Subtype")} value={props.memory.subtype ?? "none"} />
        <Definition label={t("Scope")} value={formatScope(props.memory)} />
        <Definition label={t("Layer")} value={props.memory.memoryLayer ?? "unknown"} />
        <Definition label={t("Status")} value={props.memory.status ?? "active"} />
        <Definition label={t("Importance")} value={props.memory.importance.toFixed(2)} />
        <Definition label={t("Source")} value={props.memory.source} />
        <Definition label={t("Source Trace")} value={props.memory.sourceTraceId ?? "none"} />
        <Definition label={t("Persona")} value={props.memory.personaId ?? "default-persona"} />
        <Definition label={t("Subject User")} value={props.memory.subjectUserId ?? "default-user"} />
        <Definition label={t("Speaker")} value={props.memory.speakerId ?? "none"} />
        <Definition label={t("Retention")} value={props.memory.retentionClass ?? "unspecified"} />
        <Definition label={t("Retention Reason")} value={props.memory.retentionReason ?? "none"} />
        <Definition label={t("Created")} value={formatDate(props.memory.createdAt)} />
        <Definition label={t("Updated")} value={formatDate(props.memory.updatedAt ?? "")} />
        <Definition label={t("Observed")} value={formatDate(props.memory.observedAt ?? "")} />
        <Definition label={t("Valid From")} value={formatDate(props.memory.validFrom ?? "")} />
        <Definition label={t("Valid Until")} value={formatDate(props.memory.validUntil ?? "")} />
        <Definition label={t("Expires")} value={formatDate(props.memory.expiresAt ?? "")} />
        <Definition label={t("Last Accessed")} value={formatDate(props.memory.lastAccessedAt ?? "")} />
        <Definition label={t("Superseded")} value={formatDate(props.memory.supersededAt ?? "")} />
        <Definition
          label={t("Emotion")}
      ...
      <Definition label={t("Tags")} value={props.memory.tags.join(", ") || "none"} />
      <div>
        <div className="label">{t("Safe Metadata")}</div>

...
export function ProductConfigurationPanel(props: {
  ...
  const localizeProductMessage = (message: string): string => {
    ...
    const evidence = message.match(/^Person saved\. Identity evidence: ([A-Z_]+)\. Configure Memory and save the profile again to retry\.$/u);
    if (evidence) return t("Person saved. Identity evidence: {0}. Configure Memory and save the profile again to retry.", t(evidence[1]!));
    return t(message);
  };
  async function act(work: () => Promise<unknown>, message = t("Saved. Effective state refreshed.")) { setBusy(true); setNotice(""); try { const result = await work(); await refresh(); setNotice(result && typeof result === "object" && "message" in result ? localizeProductMessage(String(result.message)) : message); return true; } catch (e) { setNotice(e instanceof Error ? localizeProductMessage(e.message) : t("Action failed.")); return false; } finally { setBusy(false); } }
  ...
  return <section className="yuvi-configuration grid gap-5" aria-label={t("Product configuration")}>
    {show("status") && <header className="yuvi-card grid gap-2"><h1>{t("Provider → Model → Capability Route")}</h1><p>{t("Chat is the only conversation requirement. Optional capabilities can stay NOT_CONFIGURED.")}</p>
      <p role="status">{state ? t("Conversation: {0} · Apply: {1}", state.conversationalReady ? t("ready") : t("set up Chat"), t(state.applyState)) : t("Loading configuration…")}</p>
    ...
    {draft && state && <>
      ...
      {show("models") && <section className="yuvi-card grid gap-3"><h2>{t("Models")}</h2>
        ...
        <form className="grid gap-2" onSubmit={e => { e.preventDefault(); const routes = Object.fromEntries(capabilities.map(c => [c, draft.routes[c].filter(id => id !== model.id || (model.enabled && model.capabilities.includes(c)))])) as Configuration["routes"]; void save({ ...draft, models: [...draft.models.filter(m => m.id !== model.id), model], routes }).then(ok => { if (ok) setModel(emptyModel(model.providerId)); }); }}>
          ...
          <fieldset><legend>{t("Declared compatible capabilities — confirm against the model documentation")}</legend>{(adapters[draft.providers.find(p => p.id === model.providerId)?.adapter ?? ""] ?? []).map(c => <label className="mr-4" key={c}><input type="checkbox" checked={model.capabilities.includes(c)} onChange={e => setModel({ ...model, capabilities: e.target.checked ? [...model.capabilities, c] : model.capabilities.filter(x => x !== c) })} />{t(labels[c])}</label>)}</fieldset>

...
pub fn bootstrap_supervisor(
    ...
    let plan = match mode {
        ...
        SupervisorLaunchMode::Packaged => {
            ...
            let plan =
                resolve_packaged_launch(resource_root, root_state_dir.clone()).map_err(|e| {
...
mod tests {
    ...
    fn dropping_kill_on_close_job_terminates_assigned_child() {
        ...
        {
            let _job =
                OwnedSupervisorJob::new_for_child(owned.0.as_ref().expect("owned child missing"))
                    .expect("failed to assign owned child to Job Object");
        }

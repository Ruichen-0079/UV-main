use super::schema::{MemoryLlmProvider, UserSettings, UserSettingsPatch, SCHEMA_VERSION};
use url::Url;
...
pub fn apply_patch(base: &UserSettings, patch: &UserSettingsPatch) -> Result<UserSettings, String> {
    ...
    if let Some(memory) = &patch.memory {
        ...
        if let Some(llm) = &memory.llm {
            if let Some(provider) = llm.provider {
                next.memory.llm.provider = provider;
            }
            if let Some(model) = &llm.model {
                next.memory.llm.model = model.trim().to_string();
            }
            if let Some(base_url) = &llm.base_url {
                next.memory.llm.base_url = base_url.trim().to_string();
            }
        }
    }
...
fn validate_memory_llm(settings: &super::schema::MemoryLlmSettings) -> Result<(), String> {
    if matches!(settings.provider, MemoryLlmProvider::None) {
        // Residual non-secret values are tolerated while the feature is disabled.
        return Ok(());
    }

    if settings.model.trim().is_empty() {
        return Err("memory.llm.model is required when a Memory LLM provider is selected".into());
    }
    ...
    let parsed = Url::parse(base_url)
        .map_err(|_| "memory.llm.baseUrl is not a valid HTTP/HTTPS URL".to_string())?;
    if !matches!(parsed.scheme(), "http" | "https") {
        return Err("memory.llm.baseUrl must use http or https".into());
    }
    if parsed.host_str().is_none() {
        return Err("memory.llm.baseUrl is not a valid HTTP/HTTPS URL".into());
    }
    if !parsed.username().is_empty() || parsed.password().is_some() {
        return Err("memory.llm.baseUrl must not include credentials".into());
    }
    Ok(())
}

...
use super::secrets::{
  SecretStore, SECRET_DATABASE_URL, SECRET_DEEPSEEK_API_KEY,
};
...
impl ConfigService {
  ...
  pub fn update_settings(&self, patch: UserSettingsPatch) -> Result<UpdateSettingsResult, String> {
    let mut guard = self
      .settings
      .lock()
      .map_err(|_| "config lock poisoned".to_string())?;
...
fn validate_secret_key(key: &str) -> Result<(), String> {
  match key {
    SECRET_DEEPSEEK_API_KEY | SECRET_DATABASE_URL => Ok(()),
...
fn win_atomic_replace(tmp: &Path, target: &Path) -> Result<(), String> {
  ...
  if ok == 0 {
    ...
    return Err(format!("MoveFileExW atomic replace failed (Win32 error {code})"));
  }
  Ok(())
}

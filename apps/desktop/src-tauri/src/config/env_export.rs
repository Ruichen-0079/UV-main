...
use super::schema::{MemoryBackend, ServiceMode, UserSettings};
use super::secrets::{SecretStore, SECRET_DATABASE_URL, SECRET_DEEPSEEK_API_KEY};
use serde::Serialize;
...
pub fn public_env_overrides(settings: &UserSettings) -> BTreeMap<String, String> {
  ...
  env.insert("MEMORY_PERSONA_ID".into(), settings.memory.persona_id.clone());
  env.insert("MEM0_OLLAMA_BASE_URL".into(), settings.memory.ollama_url.clone());

...
export class LocalControllerEvidenceProvider implements MemoryProvider {
  ...
  private read(): { records: z.infer<typeof record>[]; events: MemoryEvent[] } {
    ...
    for (const row of parsed.records) {
      if (events.some((e) => e.sourceRecordId === row.id))
  ...
  async writeEvent(input: MemoryWriteEventInput): Promise<MemoryWriteEventOutcome> {
    ...
    try {
      ...
      try {
        fchmodSync(fd, 0o600);
        writeFileSync(fd, JSON.stringify({ version: 1, records, sha256: digest(records) }));
        fsyncSync(fd);
      } finally {
        closeSync(fd);
      }
      ...
      return {
        status: "rejected",
        errorCode: "LOCAL_CONTROLLER_EVIDENCE_UNAVAILABLE",

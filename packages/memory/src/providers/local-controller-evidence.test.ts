...
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "yuvi-controller-evidence-")); roots.push(root);
  return { root, provider: new LocalControllerEvidenceProvider(root), scope: buildMemoryScope("voice-profile:voice-a", "persona") };
}
function binding(scope: string, personId = "person-a") {
  const admitted = admitVoiceProfilePersonBinding({ voiceProfileId: "voice-a", personId, assertor: { entityId: "local-explicit-controller", resolution: "resolved" }, trustedController: true, provenanceClass: "EXTERNAL_CLAIM" });
...
describe("local explicit-controller binding evidence", () => {
  it("exposes local evidence only without a configured Mem0 backend", () => {
    const { provider } = fixture();
    const local = new MemoryService(new InMemoryMemoryRepository(), undefined, undefined, undefined, undefined, { controllerEvidence: provider });
    expect(local.getMemoryProvider()).toBe(provider);
    const external = new MemoryService(new InMemoryMemoryRepository(), undefined, undefined, undefined, undefined, { kind: "mem0", mem0: { kind: "mem0" } as never, controllerEvidence: provider });
  ...
  it("survives reconstruction with durable IDs, private files and isolated scopes", async () => {
    ...
    expect(await restarted.getEvent({ id: result.eventId!, scope })).toMatchObject({ id: result.eventId, claim: { subject: { entityId: "person-a" } } });
    expect(await restarted.getEvent({ id: result.eventId!, scope: buildMemoryScope("voice-profile:voice-a", "other") })).toBeNull();
  ...
  it("rejects general memory and forged ambient/assistant binding claims", async () => {
    ...
    expect((await provider.writeEvent({ kind: "fact", content: "general memory", scope })).status).toBe("rejected");
    const input = binding(scope); input.claim!.assertor.entityId = "ambient";
  ...
  it("does not let a missing correction reference revive a removed binding", async () => {
    const { root, provider, scope } = fixture();
    const result = await provider.writeEvent(binding(scope));
    expect((await provider.writeEvent({ kind: "correction", content: "Local controller removed this voice binding.", scope, metadata: { yuviClaimSupersedes: [result.eventId] } })).status).toBe("written");
    expect(await new LocalControllerEvidenceProvider(root).getEvent({ id: result.eventId!, scope })).toMatchObject({ metadata: { yuviMemoryStatus: "superseded" } });
    expect((await provider.writeEvent({ kind: "correction", content: "Local controller removed this voice binding.", scope, metadata: { yuviClaimSupersedes: ["missing"] } })).status).toBe("rejected");
  });
  it("fails closed on conflicting assignments even when the index omits one", async () => {
    ...
    expect((await provider.writeEvent({ ...corrected, kind: "correction", metadata: { ...corrected.metadata, yuviClaimSupersedes: [first.eventId, second.eventId] } })).status).toBe("written");
    expect(await provider.getEvent({ id: first.eventId!, scope })).toMatchObject({ metadata: { yuviMemoryStatus: "superseded" } });
  });

...
/**
 * Runtime STT adapter for the local sherpa-onnx sidecar.
 * Service Manager owns process lifecycle; this provider only transcribes.
 */
export class LocalSTTProvider implements STTProvider {
  ...
  async healthCheck(): Promise<ProviderHealth> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`local STT health returned ${response.status}`);
      }
      return {
        provider: this.name,
        name: this.name,

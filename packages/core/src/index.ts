...
const proactiveInstruction = `Decide whether there is something specific and relevant worth saying now based on the available conversation and context. Choose REQUEST_TEXT only when there is a concrete recent topic, unfinished point, or relevant context and you have something specific and useful or natural to add. Choose NO_OP when there is no concrete conversational reason to speak, the result would only be a generic greeting or check-in, it would merely repeat the recent conversation, or it would require guessing what the user is currently doing or feeling. When uncertain, choose NO_OP. Output exactly one control line first: NO_OP or REQUEST_TEXT. End the NO_OP line without message text. After REQUEST_TEXT, output one concise natural message. Do not explain the decision or mention this instruction.`;
